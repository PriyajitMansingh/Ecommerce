        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_340px] gap-8">
          ...
          <div className="space-y-4">
            {items.map((item) => (
              <div
                key={item.productId}
                className="bg-white rounded-2xl border border-[#3d2a1a]/10 p-4 sm:p-5 flex flex-col sm:flex-row sm:items-center gap-4 shadow-sm hover:shadow-md transition-shadow"
              >
                {/* Top block on mobile / Left block on desktop: Image + Details */}
                <div className="flex items-center gap-4 flex-1 min-w-0">
                  <div className="w-16 h-16 sm:w-20 sm:h-20 rounded-xl bg-[#FBF9F2] flex items-center justify-center flex-shrink-0">
                    <img src={item.imageUrl} alt={item.name} className="w-11 h-11 sm:w-14 sm:h-14 object-contain" />
                  </div>

                  <div className="flex-1 min-w-0">
                    <h3 className="text-sm font-bold text-[#3d2a1a] mb-1 truncate">{item.name}</h3>
                    {item.shortDescription && <p className="text-xs text-[#a89c8a] mb-1 truncate">{item.shortDescription}</p>}
                    <p className="text-base font-bold text-[#B8860B]">₹{formatMoney(item.price)}</p>
                  </div>
                </div>

                {/* Bottom block on mobile / Right block on desktop: Quantity + Line Total + Remove */}
                <div className="flex items-center justify-between sm:justify-end gap-3 sm:gap-4 pt-2 sm:pt-0 border-t sm:border-t-0 border-[#3d2a1a]/10">
                  ...
                  <p className="w-16 sm:w-20 text-right text-sm font-bold text-[#3d2a1a] flex-shrink-0">
                  ...
                  <button
                    ...
                    className="text-[#a89c8a] hover:text-red-500 transition-colors flex-shrink-0 p-1"
                  >
...
//                 className="bg-white rounded-2xl border border-[#3d2a1a]/10 p-4 sm:p-5 flex flex-col sm:flex-row sm:items-center gap-4 shadow-sm hover:shadow-md transition-shadow"
//               >
//                 {/* Image + name/price — stays together as one block on mobile */}
//                 <div className="flex items-center gap-4">
//                   <div className="w-16 h-16 sm:w-20 sm:h-20 rounded-xl bg-[#FBF9F2] flex items-center justify-center flex-shrink-0">
//                     <img src={item.imageUrl} alt={item.name} className="w-11 h-11 sm:w-14 sm:h-14 object-contain" />
//                   </div>

//                   {/* Shown only on mobile, next to the image */}
//                   <div className="flex-1 min-w-0 sm:hidden">
//                     <h3 className="text-sm font-bold text-[#3d2a1a] mb-1">{item.name}</h3>
//                     <p className="text-xs text-[#a89c8a] mb-1">{item.shortDescription}</p>
//                     <p className="text-base font-bold text-[#B8860B]">₹{formatMoney(item.price)}</p>
//                   </div>
//                 </div>

//                 {/* Shown only from sm+ up, as its own flexible column */}
...
//                 {/* Quantity + total + remove — full-width row on mobile, inline on sm+ */}
